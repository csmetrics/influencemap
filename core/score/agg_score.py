...
AGG_FUNC = {
    'influenced_tot': np.sum,
    'influencing_tot': np.sum,
    'coauthor': np.any,
...
def agg_score_df(
    ...
    score_df['ratio_tot'] = ratio_func(score_df.influencing_tot, score_df.influenced_tot)
    score_df['ratio_tot'] = score_df.ratio_tot / score_df.sum_tot

    score_df['ratio_nsc'] = ratio_func(score_df.influencing_nsc, score_df.influenced_nsc)
    score_df['ratio_nsc'] = score_df.ratio_nsc / score_df.sum_nsc

    score_df['ratio_nca'] = ratio_func(score_df.influencing_nca, score_df.influenced_nca)
    score_df['ratio_nca'] = score_df.ratio_nca / score_df.sum_nca

    score_df['ratio_nscnca'] = ratio_func(score_df.influencing_nscnca, score_df.influenced_nscnca)
    score_df['ratio_nscnca'] = score_df.ratio_nscnca / score_df.sum_nscnca
